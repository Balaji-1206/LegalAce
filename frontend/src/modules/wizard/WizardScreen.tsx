...
const ActionPlanView: React.FC<ActionPlanViewProps> = ({ plan, scenarioTitle, lang, onRestart, showToast }) => {
  ...
  const docsList = plan?.required_documents || [];
  const authoritiesList = plan?.authorities || [];
  const templatesList = plan?.templates || [];
  ...
  const [expandedSteps, setExpandedSteps] = useState<Set<number>>(() => {
    ...
    const firstUncompleted = stepsList.findIndex((_, i) => !completedSteps.has(i));
  ...
  const handleShare = async () => {
    const text = [
      `⚖️ ${plan?.title || scenarioTitle || 'Action Plan'}`,
      '',
      ...stepsList.map(s => `${s.step_number}. ${s.title}\n   ${s.description}`),
      '',
  ...
  // suppress unused warning if function is exported/internal reference
  void handleVisitAuthority;
  ...
  const [senderEmail] = useState('user@legalace.in');
  ...
  if (!plan || stepsList.length === 0) {
    return (
      <div className="action-plan-screen" style={{ padding: '40px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: '2.5rem', marginBottom: '12px' }}>⚖️</div>
        <h3 style={{ margin: '0 0 8px', color: '#1f2937' }}>Your Action Plan is Ready</h3>
        <p style={{ color: '#6b7280', marginBottom: '24px' }}>We have recorded your answers and statutory guidance.</p>
        <button onClick={onRestart} style={{ padding: '10px 20px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 600, cursor: 'pointer' }}>
          ← Back to Situations
        </button>
      </div>
    );
  ...
  const completionPct = stepsList.length > 0
    ? Math.round((completedSteps.size / stepsList.length) * 100)
    : 0;
  ...
  return (
    <div className="action-plan-screen">
      ...
      <div className="action-plan-hero">
        ...
        <div className="action-plan-title">{plan?.title || scenarioTitle || 'Action Plan'}</div>
        <div className="action-plan-subtitle">
          {completionPct > 0
            ? `${completedSteps.size}/${stepsList.length} steps completed (${completionPct}%)`
            : `${stepsList.length} steps to resolve your issue`}
        </div>
      </div>
...
export const WizardScreen: React.FC<WizardScreenProps> = ({ userId, onBackHome }) => {
  ...
  const handleGenerateCustom = async (queryText?: string) => {
    const text = queryText || customQuery;
    if (!text.trim()) {
      showToast('info', 'Please enter your situation or legal topic.');
      return;
    }
    setGeneratingDynamic(true);
    try {
      const res = await fetch(`${BACKEND}/generate-dynamic-scenario`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.questions && data.questions.length > 0) {
        ...
        if (data.default_plan) {
          setCurrentPlan(data.default_plan);
        }
        navigateTo('questions');
        showToast('success', '🤖 Custom AI Decision Tree generated!');
      } else {
        showToast('error', 'Could not generate decision tree.');
      }
    } catch {
      showToast('error', 'Error connecting to AI scenario generator.');
    } finally {
  ...
  const createFallbackPlan = useCallback((scenario: Scenario | null): ActionPlan => {
    return {
      title: scenario ? `${scenario.title} — Action Plan` : 'Legal Action Plan',
      steps: [
        {
          step_number: 1,
          title: 'Document all evidence & correspondence',
          description: 'Gather and preserve all receipts, agreements, screenshots, emails, and notes related to your situation.',
        ...
        },
        {
          ...
          description: 'Send a formal 15-day statutory demand notice to the opposing party via Registered Post or Speed Post.',
          estimated_time: '2-3 days',
          importance: 'high',
          applicable_law: 'Indian Contract Act 1872 / Specific Relief Act'
        },
        {
          step_number: 3,
          title: 'Escalate to jurisdictional statutory authority',
          description: 'If unresolved within notice period, file a formal complaint before the competent tribunal or commission.',
      ...
      required_documents: [
        ...
        'Contracts, bills, or rent/payment receipts',
        'Written communications (emails, WhatsApp screenshots)',
        'Copy of legal demand notice + Speed Post receipt'
      ],
      authorities: [
        {
          ...
          action: 'Free legal advice & dispute mediation'
        }
      ],
      templates: [
        {
          id: 'generic_legal_notice',
          title: 'Formal Legal Demand Notice',
          description: 'Standard 15-day statutory notice template for legal disputes'
        }
      ],
      urgent: false,
      disclaimer: 'This action plan is for educational guidance only and does not constitute formal legal advice.'
    };
  }, []);
  ...
  const handleAnswersComplete = async (answers: Record<string, string>) => {
    ...
    try {
      ...
      console.error('Could not fetch quick-plan:', err);
      showToast('info', 'Showing action plan for your situation.');
    } finally {
      if (!planSet && !currentPlan) {
        setCurrentPlan(createFallbackPlan(currentScenario));
      }
      setLoading(false);
      navigateTo('plan');
    }
    ...
    <div className="wizard-screen">
      ...
      {screen === 'categories' && (
        <div key={`cat-${transitionKey}`} className={`wizard-transition-wrapper${isBack ? ' slide-back' : ''}`}>
          ...
          {/* Custom AI Situation Generator Card */}
          <div className="custom-ai-wizard-card">
            <div className="custom-ai-card-header">
              <div className="custom-ai-badge">🤖 AI Custom Guide</div>
              <h2>Don't see your exact situation?</h2>
              <p>Type what happened — AI will build a custom decision tree & action plan for you on-the-fly!</p>
            </div>
            ...
            <div className="custom-ai-input-group">
              ...
              />
              <button
                className="custom-ai-submit-btn"
                onClick={() => handleGenerateCustom()}
                disabled={generatingDynamic}
              >
                {generatingDynamic ? '⚡ Generating Plan...' : '✨ Generate AI Action Plan'}
              </button>
            </div>

            <div className="custom-ai-suggestions">
              <span className="suggestions-label">Popular Examples:</span>
              <div className="suggestions-chips-wrap">
                {[
                  ...
                  <button
                    key={idx}
                    className="custom-ai-chip-btn"
                    onClick={() => { setCustomQuery(chip); handleGenerateCustom(chip); }}
                  >
                    {chip}
                  </button>
                ))}
          ...
          <div className="wizard-section-label">{L(lang, 'categories')} ({categories.length} Categories)</div>
      ...
      {screen === 'plan' && (
        <div key={`plan-${transitionKey}`} className="wizard-transition-wrapper">
          <ActionPlanView
            plan={currentPlan || createFallbackPlan(currentScenario)}
